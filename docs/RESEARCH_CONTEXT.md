# Research context, data access and methodology

This is the detailed companion to the [project overview](../README.md).
It separates proposed economic interpretations from implemented data handling.
Original locked documents preserve the earlier specification; their hypotheses
are not established findings, and the corrections below govern current usage.

## Economic question

Can commodity data distinguish a tonne moving toward consumption from a tonne
generating repeated transactions? This project asks whether disagreement across
datasets can inform that distinction, after accounting for measurement differences.

The proposed China copper-market framework distinguishes four concepts:
...
These are distinctions to investigate, not four quantities already identified
...
| Regime | Reading |
|---|---|
| PCS > 0 | The standardized physical-block average exceeds the commercial-block average |
| PCS ≈ 0 | The two block averages are similar; individual series may still disagree |
| PCS < 0 | The standardized commercial-block average exceeds the physical-block average |

A difference of standardized averages does not establish physical corroboration,
causality, fraud, financing activity or predictive value. Both blocks can be weak
while their difference is positive; aggregation can also conceal offsetting
series. Interpreting the difference economically remains a research hypothesis.

Market pricing series are excluded from the score. A proposed future validation
would examine premiums and arbitrage measures as separate outcomes rather than
as ingredients of the predictor. That empirical confirmation analysis has not
been completed.

## Proposed validation setting

The original design proposes China's 2026 invoice-enforcement episode as a
possible setting. The frozen event configuration records April 13 and April 24
as candidate reference dates; this repository has not established their
identification validity.

The background-reading list below is a starting point for source review, not
an audited event dataset. Exact timing, provisions, enforcement intensity and
reported market responses require independent verification before empirical use.
Earlier headline claims about provision counts and premium records are not
retained as established facts in the current presentation.
...
An adequately designed study could produce a null or adverse result. The original
`PREREGISTRATION.md` lists proposed falsification conditions, but missing coverage
or an unavailable diagnostic cannot by itself establish rejection or a null effect.
...
The null-case regression test uses synthetic data with zero planted effect.
It checks a specific fixture; it does not establish false-positive control over
all data-generating processes or validate a real causal design.

Source-specific values are retained rather than forced to agree. A complete
cross-source reconciliation and discrepancy-monitoring layer is not implemented;
economic comparability still requires source-by-source review.

## Database model and evidence protection

The [warehouse model and SQL guide](DATA_MODEL.md) documents the actual
...
Start with the [offline reviewer path](../README.md#run-the-offline-demonstration).
Live ingestion and exploratory scoring are a separate path; neither is required
to exercise the synthetic estimator and regression tests.
...
- [Development lifecycle and release criteria](SDLC.md): change process,
  verification commands, test-to-requirement map, CI boundaries, and outstanding
  research and operational risks.
- [Algorithms and data contracts](ALGORITHMS.md): pipeline boundaries,
...
ingest/          fetcher contract with content-addressed raw archival
...
The proposed treatment variable is pre-existing invoice-exposure intensity by
channel, not material type. The motivation for rejecting a scrap-versus-cathode
comparison is possible substitution: the proposed control could itself respond
to the treatment. That concern motivates a different design; it does not prove
that the replacement identifies a causal effect.

The implemented synthetic demonstration exercises an exposure-intensity
estimator, tier monotonicity, pre-trends, block bootstrap, exposure-label
permutation and secondary cluster-robust errors. There is no empirical
exposure-ordered result from real copper data. Exposure measurement, valid
counterfactual assumptions, comparable outcomes and confounders remain unresolved.
Monotonicity alone cannot rule out confounding.

Measurement-uncertainty simulation remains a proposed extension, not an
implemented source of empirical evidence. The original event dates and
specification are retained for provenance, not silently relabeled as validated.
...
- People's Daily was previously linked only at its finance homepage; the exact
  release must be located and verified before it can support an event claim.
